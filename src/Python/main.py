...
def writeRequests(results, txt_file):
    ...
    for item in results:
        request = {
            'requestHeader': item['requestHeader'].replace('\r', ''),
            'requestBody': item.get('requestBody', '').replace('\r', '')
        }
...
# Spider
spidering(zap, target)

setNumBrow = zap.ajaxSpider.set_option_number_of_browsers(5)
...
txt_file = os.path.join('./src/fuzzing/resources' , 'rawRequests.txt')
